...
// ─── Monthly Availability Calendar Component ──────────────────────────────────
function MonthlyAvailCalendar({
  availOpen, setAvailOpen, availableDates, calMonth, setCalMonth,
  toggleAvailableDate, availSaved, t, C,
}: {
  availOpen: boolean;
  setAvailOpen: (fn: (v: boolean) => boolean) => void;
  availableDates: string[];
  calMonth: Date;
  setCalMonth: (fn: (v: Date) => Date) => void;
  toggleAvailableDate: (d: string) => void;
  availSaved: boolean;
  t: any;
  C: AppColors;
}) {
  ...
  const monthLabel  = calMonth.toLocaleDateString('he-IL', { month: 'long', year: 'numeric' });
  ...
  const thisMonthCount = availableDates.filter(ds => {
    ...
    <View>
      ...
      <TouchableOpacity
        ...
        style={{
          ...
          backgroundColor: availOpen ? C.blueLight : C.grayBg,
          borderRadius: 14, paddingVertical: 14, paddingHorizontal: 16,
          borderWidth: 1, borderColor: availOpen ? C.blueBorder : C.grayBorder,
        }}
      >
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
          <Text style={{ fontSize: 20 }}>📅</Text>
          <Text style={{ fontSize: 15, fontWeight: '800', color: C.textDark }}>לוח זמינות</Text>
          {availableDates.length > 0 && (
            <View style={{ backgroundColor: C.blue, borderRadius: 10, paddingHorizontal: 7, paddingVertical: 2 }}>
              <Text style={{ fontSize: 11, fontWeight: '800', color: '#fff' }}>{availableDates.length}</Text>
            </View>
          )}
        </View>
        <Text style={{ fontSize: 18, color: C.blue, fontWeight: '700' }}>
          {availOpen ? '▲' : '▼'}
        </Text>
      </TouchableOpacity>
      ...
      {/* לוח חודשי */}
      {availOpen && (
        <View style={{ marginTop: 10, backgroundColor: C.white, borderRadius: 16, padding: 14, borderWidth: 1, borderColor: C.blueBorder, gap: 8 }}>
          ...
          <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 4 }}>
            <TouchableOpacity
              ...
              style={{ padding: 8, backgroundColor: C.blueLight, borderRadius: 10 }}
            >
              <Text style={{ fontSize: 18, color: C.blue, fontWeight: '700' }}>◀</Text>
            </TouchableOpacity>
            <Text style={{ fontSize: 16, fontWeight: '900', color: C.textDark }}>{monthLabel}</Text>
            <TouchableOpacity
              onPress={() => setCalMonth(m => new Date(m.getFullYear(), m.getMonth() + 1, 1))}
              hitSlop={{ top: 10, bottom: 10, left: 10, right: 10 }}
              style={{ padding: 8, backgroundColor: C.blueLight, borderRadius: 10 }}
          ...
          <View style={{ flexDirection: 'row', marginBottom: 4 }}>
            {['א','ב','ג','ד','ה','ו','ש'].map(d => (
              <View key={d} style={{ flex: 1, alignItems: 'center' }}>
                <Text style={{ fontSize: 12, color: C.textSub, fontWeight: '700' }}>{d}</Text>
              </View>
            ))}
          </View>
              ...
              {week.map((day, di) => {
                ...
                return (
                  <TouchableOpacity
                    ...
                    onPress={() => { if (!isPast) toggleAvailableDate(ds); }}
                  ...
                  >
                    <View style={{
                      ...
                      backgroundColor: isAvail ? C.blue : 'transparent',
                      borderWidth: isPast ? 0 : 1,
                      borderColor: isAvail ? C.blue : C.grayBorder,
                      opacity: isPast ? 0.25 : 1,
                    }}>
                      <Text style={{ fontSize: 13, color: isAvail ? '#fff' : C.textDark, fontWeight: isAvail ? '900' : '400' }}>
                        {day}
                      </Text>
                    </View>
                  </TouchableOpacity>
          ...
          <View style={{ marginTop: 6, gap: 6 }}>
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
              <View style={{ width: 14, height: 14, borderRadius: 7, backgroundColor: C.blue }} />
              <Text style={{ fontSize: 11, color: C.textSub }}>
                {thisMonthCount} תאריכים זמינים בחודש זה · {availableDates.length} סה״כ
              </Text>
            </View>
            {availSaved && (
              <Text style={{ color: '#10B981', fontSize: 12, fontWeight: '800', textAlign: 'center' }}>
                ✓ {t.availSaved || 'נשמר'}
              </Text>
            )}
          </View>
...
export default function ProfileScreen() {
  ...
  // הזמנות נכנסות (מנקה) — נפתח/סגור
  const [incomingOpen, setIncomingOpen] = useState(false);

  // האם הזמנה "פגה תוקף" — שעת הסיום עברה
  const isBookingExpired = (b: any): boolean => {
    const now = new Date();
    if (b.busyUntil) return new Date(b.busyUntil) < now;
    if (b.bookingDate) {
      // אם אין busyUntil — בדוק לפי תאריך + שעות
      const d = new Date(b.bookingDate);
      const endHour = b.startTime
        ? parseInt(b.startTime.split(':')[0]) + (b.hours || 2)
        : 23;
      d.setHours(endHour, 59, 59, 0);
      return d < now;
    }
    return false;
  };
  ...
  const [paySheetTab,      setPaySheetTab]      = useState<'bit'|'paybox'>('bit');
      ...
      <View key={b.id} style={[s.bookingCard, { borderLeftWidth: 4, borderLeftColor: statusBorderColor, borderRadius: 16, padding: 0, overflow: 'hidden' }]}>
        ...
        <View style={{ paddingHorizontal: 14, paddingVertical: 10, gap: 8 }}>
          ...
          <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 6 }}>
            ...
            <View style={s.detailPill}>
              <T style={s.detailPillText}>{PAY_ICONS[b.payment] || '💳'} {b.payment === 'bit' ? t.payBit : b.payment === 'cash' ? t.payCash : b.payment === 'paybox' ? t.payPaybox : b.payment === 'card' ? t.payCard : b.payment}</T>
            </View>
  ...
  return (
    <SafeAreaView style={[s.wrap, { direction: flipSide ? 'rtl' : 'ltr' }]} edges={['left', 'right']}>
                ...
                <View style={{ backgroundColor: C.blueLight, marginHorizontal: 12, marginTop: 10, borderRadius: 16, padding: 14, gap: 6, borderWidth: 1.5, borderColor: C.blueBorder }}>
                  ...
                  <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 10 }}>
                    ...
                    <T style={{ fontSize: 12, color: C.textSub }}>
                      {PAY_ICONS[pendingConfirmBooking.payment] || '💳'} {pendingConfirmBooking.payment === 'bit' ? t.payBit : pendingConfirmBooking.payment === 'cash' ? t.payCash : pendingConfirmBooking.payment === 'paybox' ? t.payPaybox : pendingConfirmBooking.payment === 'card' ? t.payCard : pendingConfirmBooking.payment}
                    </T>
                  </View>
                </View>
                      ...
                      Alert.alert(t.cancelConfirmTitle, t.cancelConfirmMsg, [
                        ...
                        { text: t.cancelConfirmBtn, style: 'destructive', onPress: async () => {
                          try {
                            ...
                            // שלח push + הודעת צ'אט ללקוח על הדחייה
                            try {
                              const clientSnap = await getDoc(doc(db, 'users', pendingConfirmBooking.clientUid));
                              const token = clientSnap.data()?.pushToken;
                              if (token) await sendPushNotification(token, '❌ הזמנה נדחתה', `${userName} לא יכול/ה לאשר את הזמנתך — אפשר לחפש מנקה אחר`);
                              const chatId = [uid, pendingConfirmBooking.clientUid].sort().join('_');
                              await addDoc(collection(db, 'chats', chatId, 'messages'), {
                                text: `❌ מצטער/ת, לא אוכל לאשר הזמנה זו.\nאפשר לחפש מנקה אחר בעמוד הראשי.`,
                                from: 'cleaner', fromUid: uid, createdAt: new Date().toISOString(),
                              });
                              await setDoc(doc(db, 'chats', chatId), {
                                participants: [uid, pendingConfirmBooking.clientUid],
                                lastMessage: '❌ הזמנה נדחתה', lastMessageAt: new Date().toISOString(),
                                unreadBy: [pendingConfirmBooking.clientUid],
                              }, { merge: true });
                            } catch (_) {}
                          } catch (_) {}
                          setPendingConfirmBooking(null);
                ...
                <>
                  ...
                  <View style={{ backgroundColor: C.blueLight, borderRadius: 16, padding: 16, gap: 12, borderWidth: 1.5, borderColor: C.blueBorder }}>
                    ...
                    <View style={{ flexDirection: 'row', gap: 16, flexWrap: 'wrap' }}>
                      ...
                      <T style={{ fontSize: 15, fontWeight: '700', color: C.textDark }}>
                        {PAY_ICONS[b.payment] || '💳'} {b.payment === 'bit' ? t.payBit : b.payment === 'cash' ? t.payCash : b.payment === 'paybox' ? t.payPaybox : b.payment === 'card' ? t.payCard : b.payment}
                      </T>
                    </View>
                ...
                <View style={{ backgroundColor: C.blueLight, borderRadius: 16, padding: 18, gap: 13 }}>
                  ...
                  <View style={{ flexDirection: 'row', gap: 20, flexWrap: 'wrap' }}>
                    ...
                    <T style={{ fontSize: 16, fontWeight: '700', color: C.textDark }}>
                      {PAY_ICONS[confirmedBookingView.payment] || '💳'} {confirmedBookingView.payment === 'bit' ? t.payBit : confirmedBookingView.payment === 'cash' ? t.payCash : confirmedBookingView.payment === 'paybox' ? t.payPaybox : confirmedBookingView.payment === 'card' ? t.payCard : confirmedBookingView.payment}
                    </T>
                  </View>
      ...
      {loading ? (
        ...
        <ScrollView contentContainerStyle={{ paddingBottom: insets.bottom + TAB_BAR_CONTENT_HEIGHT + 16 }}>
          ...
          {isCleaner && (() => {
            const activeBookings = incomingBks.filter(b =>
              ['confirmed','onway','active'].includes(b.status) && !isBookingExpired(b)
            );
            return (
          ...
          {!isCleaner && (() => {
            const activeBookings = bookings.filter(b => {
              ...
              if (isBookingExpired(b)) return false; // פגה תוקף → היסטוריה
              return true;
            });
            return (
              <View style={[s.section, { backgroundColor: '#F0FDF4', borderRadius: 20, borderWidth: 2, borderColor: '#10B981' }]}>
          ...
          {isCleaner && (
            <View style={s.section}>
              ...
              {/* באנר התראות Push — כיבוי/הפעלה */}
              {!hasPushToken ? (
                /* ── מושבת ── */
                <View style={{
                  backgroundColor: '#FEF3C7', borderRadius: 14, padding: 14,
                  borderWidth: 1.5, borderColor: '#F59E0B', marginBottom: 14, gap: 10,
                }}>
                  <View style={{ flexDirection: 'row', alignItems: 'flex-start', gap: 10 }}>
                    <T style={{ fontSize: 22 }}>⚠️</T>
                    <View style={{ flex: 1 }}>
                      <T style={{ fontSize: 13, fontWeight: '800', color: '#92400E', marginBottom: 4 }}>
                        התראות Push מושבתות
                      </T>
                      <T style={{ fontSize: 12, color: '#B45309', lineHeight: 18 }}>
                        {'לא תוכל לקבל ניקויים דחופים ללא התראות.\nהתראות נשלחות עד השעה 20:00 בלבד.'}
                      </T>
                    </View>
                  </View>
                  <TouchableOpacity
                    ...
                    style={{
                      backgroundColor: '#D97706', borderRadius: 10, paddingVertical: 10,
                      alignItems: 'center', opacity: pushToggleLoading ? 0.6 : 1,
                    }}
                  >
                    <T style={{ color: '#fff', fontWeight: '800', fontSize: 13 }}>
                      {pushToggleLoading ? 'מפעיל...' : '🔔 הפעל התראות'}
                    </T>
                  </TouchableOpacity>
                </View>
              ) : (
                ...
                <View style={{
                  ...
                  <T style={{ fontSize: 18 }}>🔔</T>
                  <T style={{ fontSize: 11, color: '#1E40AF', flex: 1, lineHeight: 16 }}>
                    התראות פעילות · בקשות דחופות נשלחות עד 20:00
                  </T>
                  <TouchableOpacity
                    onPress={handleTogglePush}
                    disabled={pushToggleLoading}
                    style={{
                      backgroundColor: '#DBEAFE', borderRadius: 8, paddingHorizontal: 10,
                      paddingVertical: 6, borderWidth: 1, borderColor: '#93C5FD',
                      opacity: pushToggleLoading ? 0.6 : 1,
                    }}
                  >
                    <T style={{ color: '#1E40AF', fontWeight: '800', fontSize: 11 }}>
                      {pushToggleLoading ? '...' : '🔕 כבה'}
                    </T>
                  </TouchableOpacity>
                </View>
              )}
                  ...
                  {urgentRequests.map((req: any) => {
                    ...
                    return (
                      <View key={req.id} style={{ backgroundColor: '#F5F3FF', borderRadius: 16, padding: 16, marginBottom: 12, borderWidth: 2, borderColor: '#7C3AED', gap: 8 }}>
                        ...
                        <T style={{ fontSize: 12, color: C.textSub }}>
                          {PAY_ICONS[req.paymentMethod] || '💳'} {req.paymentMethod === 'cash' ? t.payCash : req.paymentMethod === 'bit' ? t.payBit : req.paymentMethod === 'paybox' ? t.payPaybox : req.paymentMethod === 'card' ? t.payCard : req.paymentMethod}
                        </T>
                        <TouchableOpacity
              ...
              {/* 3. לוח זמינות חודשי */}
              <MonthlyAvailCalendar
                availOpen={availOpen}
                setAvailOpen={setAvailOpen}
                availableDates={availableDates}
                calMonth={calMonth}
                setCalMonth={setCalMonth}
                toggleAvailableDate={toggleAvailableDate}
                availSaved={availSaved}
                t={t}
                C={C}
              />
              ...
              {/* 4. הזמנות נכנסות — נפתח */}
              {(() => {
                const nonExpired = incomingBks.filter(b => !isBookingExpired(b));
                const activeCount = nonExpired.filter(b => b.status === 'active' || b.status === 'onway').length;
                return (
                  <View>
                    {/* כותרת נפתחת */}
                    <TouchableOpacity
                      onPress={() => setIncomingOpen(o => !o)}
                      activeOpacity={0.8}
                      style={{
                        flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between',
                        backgroundColor: incomingOpen ? C.blueLight : C.grayBg,
                        borderRadius: 14, paddingVertical: 14, paddingHorizontal: 16,
                        borderWidth: 1, borderColor: incomingOpen ? C.blueBorder : C.grayBorder,
                      }}
                    >
                      <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                        <T style={{ fontSize: 20 }}>📥</T>
                        <T style={{ fontSize: 15, fontWeight: '800', color: C.textDark }}>{t.incomingBookings}</T>
                        {nonExpired.length > 0 && (
                          <View style={{ backgroundColor: C.blue, borderRadius: 10, paddingHorizontal: 7, paddingVertical: 2 }}>
                            <T style={{ fontSize: 11, fontWeight: '800', color: C.white }}>{nonExpired.length}</T>
                          </View>
                        )}
                        {activeCount > 0 && (
                          <View style={{ backgroundColor: '#10B981', borderRadius: 10, paddingHorizontal: 7, paddingVertical: 2 }}>
                            <T style={{ fontSize: 11, fontWeight: '800', color: C.white }}>🔄 {activeCount}</T>
                          </View>
                        )}
                      </View>
                      <T style={{ fontSize: 18, color: C.blue, fontWeight: '700' }}>
                        {incomingOpen ? '▲' : '▼'}
                      </T>
                    </TouchableOpacity>
                    ...
                    {incomingOpen && (
                      <View style={{ marginTop: 10 }}>
                        {nonExpired.length === 0 ? (
                          ...
                          nonExpired.map(b => renderBookingCard(b, true))
                        )}
                      </View>
                    )}
                  </View>
              ...
              <View style={{ marginBottom: 16 }}>
                ...
                {portfolioSaved && (
                  <View style={{
                    ...
                    <T style={{ color: '#15803D', fontWeight: '800', fontSize: 13 }}>✅ התמונה נשמרה!</T>
                  </View>
                )}
              </View>
          ...
          {!isCleaner && (() => {
            const historyBookings = bookings.filter(b => {
              // סטטוס סגור (done/cancelled וכד') → תמיד בהיסטוריה
              if (!['pending','confirmed','onway','active'].includes(b.status)) return true;
              // הזמנה "פעילה" שפג תוקפה → גם בהיסטוריה
            ...
            return (
              <View style={s.section}>
                ...
                <TouchableOpacity
                  ...
                  style={{
                    flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between',
                    backgroundColor: historyOpen ? C.blueLight : C.grayBg,
                    borderRadius: 14, paddingVertical: 14, paddingHorizontal: 16,
                    borderWidth: 1, borderColor: historyOpen ? C.blueBorder : C.grayBorder,
                  }}
                >
                  <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                    <T style={{ fontSize: 20 }}>📋</T>
                    <T style={{ fontSize: 15, fontWeight: '800', color: C.textDark }}>{t.historyTitle}</T>
                    {historyBookings.length > 0 && (
                      <View style={{ backgroundColor: C.blue, borderRadius: 10, paddingHorizontal: 7, paddingVertical: 2 }}>
                        <T style={{ fontSize: 11, fontWeight: '800', color: C.white }}>{historyBookings.length}</T>
                      </View>
                    )}
                  </View>
                  <T style={{ fontSize: 18, color: C.blue, fontWeight: '700' }}>
              ...
              {(['bit','paybox'] as const).map(tab => (
                ...
                >
                  <T style={{ fontSize: 12, fontWeight: '800', color: paySheetTab === tab ? C.white : C.textDark }}>
                    {tab === 'bit' ? t.paySheetBitTab : t.paySheetPayboxTab}
                  </T>
                </TouchableOpacity>
              ))}
            </View>
            ...
            <ScrollView contentContainerStyle={{ padding: 20, gap: 14, paddingBottom: insets.bottom + 16 }}>
              ...
              <View>
                ...
                <View style={{ flexDirection: 'row', gap: 8, marginTop: 6 }}>
                  {[{ key: 'cash', label: '💵 ' + t.payCash }, { key: 'bit', label: '📱 ' + t.payBit }, { key: 'paybox', label: '💜 ' + t.payPaybox }, ].map(p => (
