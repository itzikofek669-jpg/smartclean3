/**
 * Map styling, tied to the app's own theme rather than the phone's.
 *
 * The Google Maps SDK follows the system dark setting on its own. The app does
 * not — it keeps its own light/dark flag (ThemeContext, stored per device and
 * defaulting to light). So a phone in system dark mode drew a dark map inside a
 * light app: the one dark rectangle on the home screen, which is what it looked
 * like from the outside.
 *
 * Passing a style at all is what settles it — the SDK stops applying its
 * automatic night mode the moment a MapStyleOptions is supplied, so the map
