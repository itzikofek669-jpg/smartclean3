...
import { pendingMove, publicCity, publicCoord, reconcile, splitFields } from './profileFields';
...
export async function fetchOwnProfile(uid: string): Promise<Record<string, any> | null> {
  if (!uid) return null;
  const [snap, priv] = await Promise.all([getDoc(doc(db, 'users', uid)), fetchPrivateProfile(uid)]);
  if (!snap.exists()) return null;
  return { ...snap.data(), ...priv };
}
...
export async function migrateProfile(uid: string, data: Record<string, any> | null | undefined): Promise<boolean> {
  ...
  try {
    ...
    for (const k of keys) {
      const keep = reconcile(data[k], existing[k]);
    ...
    if (cityMoves) {
      const keep = reconcile(rawCity, existing.city);
